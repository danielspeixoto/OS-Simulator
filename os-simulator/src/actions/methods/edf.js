...
const edf = (processes, memoryMethod, quantum) => {
	...
	if(memoryMethod === 'fifo') {
		...
		for(j = 0; j < processesClone.length;){
				...
				// Loading pages lasts a quantum for each page, only load the pages that are not in the memory
				for(i = 0; i < quantum * (aux.numOfPages - aux.loadedPages);) {				
					var menor = time
					//Looks for the least recent page and get its index
					for(k = 0; k < 100; k++){
						if(ages[k] === null){
							pointerToPage = k
							break
				...
				//update the age of all the pages of the most recently refered page
				for(k = 0; k < 100; k++){
